...
/**
 * GRAMMAR HIGHLIGHTS registry — the curated guided walkthroughs.
 *
 * Authoring rules (see also the schema notes in `domain/schema/guided.ts`):
 * - Author against REAL ids only (`npm run dump-syntax -- 'sblgnt:<ref>'`),
 *   for passages present in the guided bundle (`src/data/guidedPassages.ts` +
 *   `npm run guided:build`); validate with `npm run guided:check`.
 * - Tone: Bible-study/devotional. English translations are affirmed — the
 *   guide shows WHY translators make the choices they do, it never suggests
 *   Greek is a secret code that invalidates an English Bible.
 ...
 * - Contested passages get a `debateSummary` with FAIR summaries of the major
 *   readings; grammar opens and constrains the question, it rarely settles a
 *   doctrine by itself.
 ...
 *
 * STATUS: the Hebrews guide below is the v1 SAMPLE used to build and test the
 * guided-mode infrastructure. The full guide list (Mark 5:25–34, 1 Pet
 * 3:18–22, Matt 28:19–20, Matt 6:11 ∥ Luke 11:3, 1 John 2:1 ∥ 3:6–9, …) is
 * PROPOSED in docs/agent-handoff-grammar-highlights.md and awaits Tim's
 * approval before authoring.
 */

const hebrews1: GrammarHighlightGuide = {
...
export const grammarHighlightsRegistry: GrammarHighlightsRegistry =
  GrammarHighlightsRegistrySchema.parse({
    version: 1,
    guides: [hebrews1],
  });
