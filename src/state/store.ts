...
// `replaceFiller` / `reattachNode` (the single-occupancy subject/predicate
// slot-swap and the attach transformation) live in `domain/model/mutations` so
// the Phrase/Block drag PREVIEW applies the exact same pure move.
...
export const useEditorStore = create<EditorStore>((set, get) => {
  ...
  return {
    ...
    attachNodeTo: (dependentId, headId, type) => {
      ...
      // The pure transformation (re-point + slot swap) is shared with the
      // Phrase/Block drag preview — see `reattachNode` in domain/model.
      commit((d) => ({ ...d, syntax: reattachNode(d.syntax, dependentId, headId, type) }));
    },
